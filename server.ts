...
async function startServer() {
  ...
  // ==========================================
  // 1b. AI Smart Auth & User Role Classifier
  // ==========================================
  app.post("/api/v1/ai/auth-classify", async (req, res) => {
    try {
      const { identifier, context, country } = req.body || {};
      const raw = (identifier || "").trim().toLowerCase();

      // Quick rule-based matching for deterministic accounts
      let matchedRole: 'passenger' | 'driver' | 'admin' = 'passenger';
      let matchedConfidence = 85;

      if (raw === 'ahmaidat' || raw === 'admin' || raw.includes('admin') || raw.includes('supervisor')) {
        matchedRole = 'admin';
        matchedConfidence = 99;
      } else if (raw.startsWith('drv_') || raw.includes('driver') || raw.includes('captain') || raw === 'khalil' || raw === '0791112233') {
        matchedRole = 'driver';
        matchedConfidence = 98;
      } else if (raw.startsWith('psg_') || raw.includes('passenger') || raw === 'ahmad' || raw === '0780001122') {
        matchedRole = 'passenger';
        matchedConfidence = 98;
      }
      ...
      const fallback = {
        ...
        arabicRoleName: matchedRole === 'admin' ? 'مدير المنظومة والعمليات' : (matchedRole === 'driver' ? 'كابتن سائق معتمد' : 'راكب ومستخدم التطبيق'),
        welcomeMessage: matchedRole === 'admin' 
          ? 'مرحباً بك في مركز القيادة والتحكم الإداري لمنظومة آدم.'
          : (matchedRole === 'driver' ? 'أهلاً بك يا كابتن! جاهزون لاستقبال طلبات الركاب وتحقيق أرباحك اليومية.' : 'أهلاً وسهلاً بك في تطبيق آدم! أين وجهتك القادمة اليوم؟'),
        suggestedService: matchedRole === 'admin' ? 'dashboard' : (matchedRole === 'driver' ? 'radar' : 'instant_ride'),
        targetView: matchedRole === 'admin' ? 'admin' : (matchedRole === 'driver' ? 'driver' : 'passenger'),
        quickActions: matchedRole === 'admin' 
          ? ['متابعة الرحلات الحية', 'تدقيق طلبات الكباتن', 'إدارة التسعيرة والمحافظ']
          : (matchedRole === 'driver' ? ['تفعيل وضع الاستعداد للطلبات', 'عرض المحفظة والأرباح', 'تشغيل عداد التكسي الذكي'] : ['طلب تكسي فوري', 'حجز رحلة بين المحافظات', 'شحن محفظة آدم']),
        aiTip: 'تم التعرف على نوع الحساب وتوجيهك تلقائياً للواجهة المخصصة بكامل ميزاتها.'
      };

      if (!raw || !ai) {
        return res.json({ success: true, classification: fallback });
      }

      const systemInstruction = `أنت المساعد الذكي لنظام توثيق الدخول وإدارة الهويات في منظومة "آدم" الذكية للنقل التشاركي وتوصيل الركاب.
مهمتك: تصنيف هوية المستخدم أو اسم الدخول أو رقم هاتفه وتحديد الدور التشغيلي الأنسب له:
1. 'passenger' (راكب يبحث عن طلب رحلات وتوصيل وتكسي)
2. 'driver' (كابتن وسائق يقدم خدمة التوصيل ولديه مركبة)
3. 'admin' (مسؤول إداري ومدير للعمليات)

أعد JSON فقط بهذا الهيكل:
{
  "role": "passenger" | "driver" | "admin",
  "confidence": 95,
  "arabicRoleName": "اسم الدور بالعربية",
  "welcomeMessage": "رسالة ترحيب مخصصة مشجعة بالدور",
  "suggestedService": "instant_ride" | "radar" | "dashboard",
  "targetView": "passenger" | "driver" | "admin",
  "quickActions": ["إجراء 1", "إجراء 2", "إجراء 3"],
  "aiTip": "نصيحة ذكية موجهة للمستخدم"
}`;

      const prompt = `بيانات الدخول المطلوب تحليلها وتصنيفها:
- المعرف / اسم المستخدم / الهاتف: "${raw}"
- السياق الإضافي: ${JSON.stringify(context || {})}
- الدولة: ${country || 'الأردن (JO)'}`;

      const classification = await generateGeminiJson(prompt, systemInstruction, fallback);
      res.json({ success: true, classification: classification || fallback });
    } catch (err: any) {
