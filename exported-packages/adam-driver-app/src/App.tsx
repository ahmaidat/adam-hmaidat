...
function DashboardContent() {
  ...
  const resolveViewFromLocation = (): 'welcome' | 'passenger' | 'driver' | 'admin' | 'dashboard' | 'simulator' => {
    try {
      ...
      if (pathname === '/' || pathname === '') {
        ...
        if (saved && ['welcome', 'passenger', 'driver', 'admin', 'dashboard', 'simulator'].includes(saved)) {
          return saved as any;
        }
      }
    } catch {}
  ...
  const [activeView, setActiveViewState] = useState<'welcome' | 'passenger' | 'driver' | 'admin' | 'dashboard' | 'simulator'>(() => resolveViewFromLocation());

  const setActiveView = (view: 'welcome' | 'passenger' | 'driver' | 'admin' | 'dashboard' | 'simulator') => {
    setActiveViewState(view);
    try {
      localStorage.setItem('adam_active_view', view);
      let targetPath = '/';
      if (view === 'passenger') targetPath = '/passenger';
