...
interface AppContextProps {
  ...
  login: (username: string, password: string, role?: 'driver' | 'passenger' | 'admin' | 'auto') => { success: boolean; msg: string; user?: any; role?: 'driver' | 'passenger' | 'admin' };
  ...
  setDriverServiceScope: (driverId: string, scope: 'all' | 'intracity' | 'intercity' | 'scheduled', allowedServices?: ('intracity' | 'intercity' | 'scheduled')[]) => void;
  setPassengerServiceScope: (passengerId: string, scope: 'all' | 'intracity' | 'intercity' | 'scheduled', allowedServices?: ('intracity' | 'intercity' | 'scheduled')[]) => void;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Login handler with Unified Auto-Detection across all roles
  const login = (usernameOrPhone: string, psword: string, role?: 'driver' | 'passenger' | 'admin' | 'auto') => {
    const rawInput = (usernameOrPhone || '').trim();
    const inputUserLower = rawInput.toLowerCase();
    const targetRole = role || 'auto';

    // Helper: Attempt Admin/Employee Login
    const attemptAdminLogin = () => {
      ...
      if (inputUserLower === 'ahmaidat' || inputUserLower === 'admin' || inputUserLower === storedAdminUser) {
        ...
        return { success: true, msg: 'تم الدخول بنجاح كمدير للنظام كابتن Ahmaidat', user: u, role: 'admin' as const };
      }
      
      const matchedEmp = employees.find(e => e.username.toLowerCase() === inputUserLower);
      if (matchedEmp) {
        ...
        if (correctPassword === psword) {
          ...
          return { success: true, msg: `تم الدخول بنجاح كموظف: ${matchedEmp.fullName}`, user: matchedEmp, role: 'admin' as const };
        } else {
    ...
    // Helper: Attempt Passenger Login
    const attemptPassengerLogin = () => {
      const passenger = passengers.find(p => 
        p.username === rawInput || 
        p.phone === rawInput || 
        p.username.toLowerCase() === inputUserLower ||
        (p.email && p.email.toLowerCase() === inputUserLower)
      );
      if (passenger) {
        const correctPassword = passenger.password || '123';
        if (correctPassword !== psword) {
          return { success: false, msg: 'كلمة المرور لحساب الراكب غير صحيحة. يرجى مراجعة رسائل SMS أو إعادة التعيين.' };
        }
        if (passenger.status === 'blocked') {
          return { success: false, msg: 'تنبيه: تم حظر حساب الراكب من قبل إدارة نظام آدم بسبب مخالفة الشروط.' };
        }
        if (passenger.status === 'pending') {
          return { success: false, msg: 'حسابك كـ راكب في قيد المراجعة الإدارية للتحقق من هوية الراكب.' };
        }
        ...
        return { success: true, msg: `أهلاً بك يا ${passenger.fullName}! تم تسجيل الدخول كـ راكب بنجاح.`, user: passenger, role: 'passenger' as const };
      }
      return null;
    };

    // Helper: Attempt Driver Login
    const attemptDriverLogin = () => {
      const driver = drivers.find(d => 
        d.username === rawInput || 
        d.phone === rawInput || 
        d.username.toLowerCase() === inputUserLower ||
        (d.email && d.email.toLowerCase() === inputUserLower)
      );
      if (driver) {
        const correctPassword = driver.password || '123';
        if (correctPassword !== psword) {
          return { success: false, msg: 'كلمة المرور لحساب الكابتن غير صحيحة. يرجى التحقق من الرسالة النصية أو كلمة المرور الحالية.' };
        }
        if (driver.status === 'blocked') {
          return { success: false, msg: 'تنبيه: تم حظر حساب الكابتن من قبل إدارة نظام آدم بسبب مخالفة الشروط.' };
        }
        if (driver.status === 'pending') {
          return { success: false, msg: 'تم إرسال مستنداتك بنجاح ولكن حساب الكابتن قيد المراجعة والتدقيق الإداري من لوحة التحكم.' };
        }
        ...
        return { success: true, msg: `أهلاً بك كابتن ${driver.fullName}! تم تسجيل الدخول بنجاح.`, user: driver, role: 'driver' as const };
      }
      return null;
    };
    ...
    if (targetRole === 'admin') {
      const res = attemptAdminLogin();
      return res || { success: false, msg: 'اسم مستخدم المسؤول أو الموظف غير صحيح' };
    }

    if (targetRole === 'passenger') {
      const res = attemptPassengerLogin();
      return res || { success: false, msg: 'اسم المستخدم أو رقم الهاتف غير مسجل كـ راكب في تطبيق آدم.' };
    }

    if (targetRole === 'driver') {
      const res = attemptDriverLogin();
      return res || { success: false, msg: 'اسم المستخدم أو رقم الهاتف غير مسجل كـ كابتن سائق في نظام آدم.' };
    }

    // Auto-detect Role flow ('auto')
    // 1. Check Passenger first
    const psgRes = attemptPassengerLogin();
    if (psgRes) return psgRes;

    // 2. Check Driver
    const drvRes = attemptDriverLogin();
    if (drvRes) return drvRes;

    // 3. Check Admin / Employee
    const admRes = attemptAdminLogin();
    if (admRes) return admRes;

    return { 
      success: false, 
      msg: 'اسم المستخدم أو رقم الهاتف غير مسجل في منظومة آدم. يرجى إنشاء حساب راكب أو كابتن جديد.' 
    };
  };
...
⏳ الحساب قيد التدقيق الإداري: بانتظار مراجعة وتفعيل الحساب يدوياً من قبل الإدارة من لوحة التحكم قبل السماح بالدخول.`;
  ...
  // Set driver service scope (all, intracity, intercity, scheduled)
  const setDriverServiceScope = (
    driverId: string,
    scope: 'all' | 'intracity' | 'intercity' | 'scheduled',
    allowedServices?: ('intracity' | 'intercity' | 'scheduled')[]
  ) => {
    const updatedDrivers = drivers.map(d => {
      if (d.id === driverId) {
        const services = allowedServices || (scope === 'all' ? ['intracity', 'intercity', 'scheduled'] : [scope]);
        return {
          ...d,
          serviceScope: scope,
          allowedServices: services,
          workScope: scope === 'intracity' ? ('local' as const) : scope === 'intercity' ? ('intercity' as const) : ('both' as const)
        };
      }
      return d;
    });
  ...
  // Set passenger service scope (all, intracity, intercity, scheduled)
  const setPassengerServiceScope = (
    passengerId: string,
    scope: 'all' | 'intracity' | 'intercity' | 'scheduled',
    allowedServices?: ('intracity' | 'intercity' | 'scheduled')[]
  ) => {
