...
export const DriverApp: React.FC<DriverAppProps> = ({ fullWidth: initialFullWidth = false }) => {
  ...
  useEffect(() => {
    ...
    ApiService.getVehicles()
      .then(data => {
        if (isMounted && data && data.success && data.brands && data.brands.length > 0) {
        ...
        {!loggedDriver ? (
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            <div className="px-4 py-2 bg-slate-950 border-b border-slate-850 flex justify-between items-center">
              ...
              <div className="text-right flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {(!settings.uiControls?.hideHomeButton && !settings.uiControls?.hideDriverHomeButton) && (
                    <button
                      type="button"
                      onClick={() => {
                        setActiveTab('status');
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-5 flex flex-col gap-4 text-right transition-all" id="driver-scroll-content">
              ...
              {/* WELCOME GREETING BANNER FOR CAPTAIN PER ADMIN CONFIG */}
              {!settings.uiControls?.hideWelcomeGreeting && !settings.uiControls?.hideDriverWelcomeGreeting && (
                <div className="bg-gradient-to-r from-amber-950/50 via-slate-900 to-amber-950/30 border border-amber-500/25 p-3 rounded-2xl flex items-center justify-between shadow-md animate-fadeIn text-right">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="flex flex-col gap-0.5">
                      <h4 className="text-xs font-bold text-slate-100 flex items-center gap-1.5">
                        <span>🚗</span>
                        <span>{settings.uiControls?.driverWelcomeText || `مرحباً بك كابتن ${loggedDriver.fullName.split(' ')[0]} 👋`}</span>
                      </h4>
                      <p className="text-[10px] text-slate-400 leading-tight">
                        {t('نتمنى لك يوماً موفقاً ورحلات مباركة مع منظومة آدم بالأردن', 'Wishing you a productive day & profitable rides with Adam')}
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 text-right">
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3.5 flex flex-col gap-3">
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-1 text-right">
                        <label className="text-[9px] text-slate-500 pr-1">رقم هاتف الكابتن</label>
                        <input
                          type="tel"
                          required
                          value={editPhone}
                          onChange={e => setEditPhone(e.target.value)}
                          className="bg-slate-950/60 border border-slate-850 focus:border-amber-550 text-xs text-slate-100 p-2 rounded-xl outline-none font-mono text-left font-bold"
                        />
                      </div>
                      ...
                      <div className="flex flex-col gap-1 text-right">
                        ...
                        <input
                          ...
                          className="bg-slate-950/60 border border-slate-850 focus:border-amber-550 text-xs text-slate-100 p-2 rounded-xl outline-none font-mono text-left"
                        />
                      </div>

                      <div className="flex flex-col gap-1 text-right">
                        <label className="text-[9px] text-slate-500 pr-1">نوع وموديل ولون المركبة</label>
                        <input
                          type="text"
                          required
                          value={editCarDesc}
                          onChange={e => setEditCarDesc(e.target.value)}
                          className="bg-slate-950/60 border border-slate-850 focus:border-amber-550 text-xs text-slate-100 p-2 rounded-xl outline-none font-sans text-right"
                        />
                      </div>
                      ...
                      <button
                        type="submit"
                        className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-black py-2 rounded-xl text-xs transition cursor-pointer shadow mt-2"
                      >
                        حفظ التعديلات في ملف الكابتن 💾
                      </button>
                    </form>
                  </div>
                </div>
            ...
            <div className="bg-slate-900 border-t border-slate-800/80 px-2 py-2 flex items-center justify-around z-30 shrink-0">
              {(!settings.uiControls?.hideHomeButton && !settings.uiControls?.hideDriverHomeButton) ? (
                <button
                  ...
                  className={`flex flex-col items-center gap-1 transition ${activeTab === 'status' && !isDriveMode ? 'text-amber-400' : 'text-slate-400 hover:text-slate-200'}`}
                >
                ...
                <button
                  ...
                  className={`flex flex-col items-center gap-1 transition ${activeTab === 'status' ? 'text-amber-400' : 'text-slate-400 hover:text-slate-200'}`}
                >
                  <Car className="w-5 h-5" />
                  <span className="text-[10px] font-bold font-sans">الرحلات والعمليات</span>
                </button>
              )}
              ...
              >
                ...
                <span className="text-[10px] font-bold font-sans">المجدولة والمدن</span>
              </button>
