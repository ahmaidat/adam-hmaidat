    ...
    <div className="flex flex-col gap-6 text-right font-sans" dir="rtl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-lg flex flex-col gap-4">
          ...
          <div className="bg-gradient-to-r from-amber-950/30 via-slate-950 to-slate-950 border border-amber-500/40 rounded-xl p-3.5 flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <div className="flex flex-col gap-0.5">
                <div className="flex items-center gap-1.5 text-xs font-bold text-amber-300">
                  <Navigation className="w-3.5 h-3.5 text-amber-400" />
                  <span>أيقونة وزر الرئيسية (Home Icon & Button)</span>
                </div>
                <span className="text-[10px] text-slate-400">
                  {uiControls.hideHomeButton ? 'مخفية كلياً من تطبيق الراكب والكابتن لمنع التشتت' : 'ظاهرة ومفعلة في الواجهات'}
                </span>
              </div>
            ...
            {/* Granular Home Icon controls per app */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2 border-t border-slate-800/80">
              <div className="bg-slate-900/80 p-2 rounded-lg flex items-center justify-between">
                <span className="text-[10.5px] text-slate-300 font-bold">أيقونة الرئيسية للراكب:</span>
                <button
                  type="button"
                  onClick={() => handleToggleUiControl('hidePassengerHomeButton')}
                  className={`text-[10px] font-bold px-2 py-0.5 rounded transition ${
                    uiControls.hidePassengerHomeButton ? 'bg-rose-950/80 text-rose-300 border border-rose-800' : 'bg-emerald-950/80 text-emerald-300 border border-emerald-800'
                  }`}
                >
                  {uiControls.hidePassengerHomeButton ? 'مخفي' : 'ظاهر'}
                </button>
              </div>

              <div className="bg-slate-900/80 p-2 rounded-lg flex items-center justify-between">
                <span className="text-[10.5px] text-slate-300 font-bold">أيقونة الرئيسية للكابتن:</span>
                <button
                  type="button"
                  onClick={() => handleToggleUiControl('hideDriverHomeButton')}
                  className={`text-[10px] font-bold px-2 py-0.5 rounded transition ${
                    uiControls.hideDriverHomeButton ? 'bg-rose-950/80 text-rose-300 border border-rose-800' : 'bg-emerald-950/80 text-emerald-300 border border-emerald-800'
                  }`}
                >
                  {uiControls.hideDriverHomeButton ? 'مخفي' : 'ظاهر'}
                </button>
              </div>
            </div>
          </div>

          {/* Welcome Greeting Banner Control */}
          <div className="bg-gradient-to-r from-indigo-950/30 via-slate-950 to-slate-950 border border-indigo-500/40 rounded-xl p-3.5 flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <div className="flex flex-col gap-0.5">
                <div className="flex items-center gap-1.5 text-xs font-bold text-indigo-300">
                  <Sparkles className="w-3.5 h-3.5 text-indigo-400" />
                  <span>الكلمة والعبارة الترحيبية (Welcome Greeting Banner)</span>
                </div>
                <span className="text-[10px] text-slate-400">
                  {uiControls.hideWelcomeGreeting ? 'مخفية حالياً في تطبيق الراكب والكابتن' : 'ظاهرة ومفعلة أعلى الشاشات'}
                </span>
              </div>

              <button
                type="button"
                onClick={() => handleToggleUiControl('hideWelcomeGreeting')}
                className={`px-3 py-1.5 rounded-xl text-xs font-black transition duration-150 flex items-center gap-1.5 cursor-pointer shadow-md ${
                  uiControls.hideWelcomeGreeting 
                    ? 'bg-rose-600 hover:bg-rose-700 text-white' 
                    : 'bg-emerald-600 hover:bg-emerald-700 text-white'
                }`}
              >
            ...
            {/* Granular Welcome Greeting controls per app */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2 border-t border-slate-800/80">
              <div className="bg-slate-900/80 p-2 rounded-lg flex items-center justify-between">
                <span className="text-[10.5px] text-slate-300 font-bold">ترحيب الراكب:</span>
                <button
                  type="button"
                  onClick={() => handleToggleUiControl('hidePassengerWelcomeGreeting')}
                  className={`text-[10px] font-bold px-2 py-0.5 rounded transition ${
                    uiControls.hidePassengerWelcomeGreeting ? 'bg-rose-950/80 text-rose-300 border border-rose-800' : 'bg-emerald-950/80 text-emerald-300 border border-emerald-800'
                  }`}
                >
                  {uiControls.hidePassengerWelcomeGreeting ? 'مخفي' : 'ظاهر'}
                </button>
              </div>

              <div className="bg-slate-900/80 p-2 rounded-lg flex items-center justify-between">
                <span className="text-[10.5px] text-slate-300 font-bold">ترحيب الكابتن:</span>
                <button
                  type="button"
                  onClick={() => handleToggleUiControl('hideDriverWelcomeGreeting')}
                  className={`text-[10px] font-bold px-2 py-0.5 rounded transition ${
                    uiControls.hideDriverWelcomeGreeting ? 'bg-rose-950/80 text-rose-300 border border-rose-800' : 'bg-emerald-950/80 text-emerald-300 border border-emerald-800'
                  }`}
                >
                  {uiControls.hideDriverWelcomeGreeting ? 'مخفي' : 'ظاهر'}
                </button>
              </div>
            </div>

            {/* Custom Welcome Message Inputs */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
              <div className="flex flex-col gap-1">
                <label className="text-[9.5px] text-slate-400 font-bold">نص ترحيب الراكب (اختياري):</label>
                <input
                  type="text"
                  placeholder="مرحباً بك، نتمنى لك رحلة آمنة ومريحة"
                  value={uiControls.passengerWelcomeText || ''}
                  onChange={(e) => {
                    const updated = { ...uiControls, passengerWelcomeText: e.target.value };
                    updateSettings({ uiControls: updated });
                  }}
                  className="bg-slate-950/80 border border-slate-800 text-[11px] text-slate-200 p-1.5 rounded-lg outline-none focus:border-indigo-500"
                />
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-[9.5px] text-slate-400 font-bold">نص ترحيب الكابتن (اختياري):</label>
                <input
                  type="text"
                  placeholder="مرحباً كابتن، نتمنى لك يوماً موفقاً ورحلات مباركة"
                  value={uiControls.driverWelcomeText || ''}
                  onChange={(e) => {
                    const updated = { ...uiControls, driverWelcomeText: e.target.value };
                    updateSettings({ uiControls: updated });
                  }}
                  className="bg-slate-950/80 border border-slate-800 text-[11px] text-slate-200 p-1.5 rounded-lg outline-none focus:border-indigo-500"
                />
