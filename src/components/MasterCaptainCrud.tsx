...
export const MasterCaptainCrud: React.FC = () => {
  ...
  const handleEditClick = (driver: Driver) => {
    ...
    setEditServiceScope(driver.serviceScope || 'all');
    setEditAllowedServices(driver.allowedServices && driver.allowedServices.length > 0 ? driver.allowedServices : ['intracity', 'intercity', 'scheduled']);
    ...
    const updated = drivers.map(d => {
      if (d.id === id) {
        return {
          ...
          workScope: editServiceScope === 'intracity' ? ('local' as const) : editServiceScope === 'intercity' ? ('intercity' as const) : ('both' as const)
        };
      }
      return d;
    });
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    const newDriver: Driver = {
      ...
      workScope: addServiceScope === 'intracity' ? 'local' : addServiceScope === 'intercity' ? 'intercity' : 'both',
    ...
    <div className="space-y-6 text-right font-sans" dir="rtl">
      ...
      {showAddForm && (
        <form onSubmit={handleAddSubmit} className="bg-[#0c142c]/70 p-4 rounded-xl border border-amber-500/25 space-y-4">
          ...
          {/* Service Permissions / Allowed Services for Captain */}
          <div className="bg-[#05070e]/60 p-3 rounded-lg border border-slate-800 space-y-2">
            <div className="flex justify-between items-center flex-row-reverse">
              <span className="text-[11px] font-bold text-amber-400">🛡️ {t('صلاحيات ونطاق الخدمات الممنوحة للكابتن:', 'Allowed Services & Service Permissions:')}</span>
          ...
          <div key={driver.id} className="bg-slate-900/40 border border-slate-800 rounded-xl p-4 transition-all space-y-3">
            ...
            {editingDriverId === driver.id ? (
              <div className="bg-[#05070e] p-3 rounded-lg border border-slate-800 space-y-3 text-xs">
                ...
                {/* Edit Service Permissions for Driver */}
                <div className="bg-[#080d1a] p-2.5 rounded-lg border border-slate-800 space-y-2">
                  <div className="flex justify-between items-center flex-row-reverse">
                    <span className="text-[11px] font-bold text-amber-400">🛡️ {t('صلاحيات ونطاق الخدمات الممنوحة للكابتن:', 'Service Permissions:')}</span>
                    <span className="text-[10px] text-slate-400 font-sans">
              ...
              <div className="space-y-2 text-xs">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 bg-[#05070e]/50 p-2.5 rounded-lg text-slate-300">
                  ...
                  <div className="col-span-2 md:col-span-3 bg-slate-950/60 p-2 rounded-lg border border-slate-850 flex items-center justify-between flex-row-reverse">
                    <span className="text-[10px] font-bold text-amber-400">🛡️ {t('الخدمات المتاحة للكابتن:', 'Allowed Services:')}</span>
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10.5px] font-bold bg-amber-950/50 border border-amber-500/30 text-amber-300">
                      {driver.serviceScope === 'intracity' ? '🏢 داخل المدينة فقط' : driver.serviceScope === 'intercity' ? '🛣️ بين المحافظات فقط' : driver.serviceScope === 'scheduled' ? '⏰ رحلات مجدولة فقط' : '✨ جميع الخدمات (مفتوح)'}
                    </span>
                  </div>
                </div>
